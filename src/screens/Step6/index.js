...
export default function Language({setScreen}) {
  ...
  const _updateSurveyResult = value => {
    dispatch(updateSurveyResult(`step6,${value}`));
  };
    ...
    <>
      ...
      <View style={{flex: 1, backgroundColor: '#963D22'}}>
        <View style={styles.imageContainer}>
          <Image
            source={require('../../assets/survey_6.jpeg')}
            style={styles.image}
          />
        </View>
        ...
        <ImageBackground
          ...
          <Animated.View style={{opacity: fadeIn}}>
            <Text style={{fontSize: 38, color: '#F9A53A', textAlign: 'center'}}>
              {lang == 'en'
                ? 'How was your experience at our '
                : 'ما رأيك في منطقة '}
              <Text
                style={{fontSize: lang == 'en' ? 44 : 40, color: '#e38f24'}}>
                {lang == 'en' ? 'kid’s area?' : 'ألعاب الأطفال لدينا؟'}
              </Text>
            </Text>
